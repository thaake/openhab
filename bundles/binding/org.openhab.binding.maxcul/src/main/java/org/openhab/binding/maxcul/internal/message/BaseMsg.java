...
import org.openhab.binding.maxcul.dep.internal.message.sequencers.MessageSequencer;
...
public class BaseMsg {
	...
	public static boolean isForUs(String rawMsg, String addr) {
		...
		String targetAddress = rawMsg.substring(PKT_POS_DST_ADDR,
				PKT_POS_DST_ADDR + PKT_POS_DST_ADDR_LEN);
		
		return targetAddress.equals(addr) || targetAddress.equals(MaxCulBinding.BROADCAST_ADDRESS);
	}
	
	public static BaseMsg parseMessage(String rawMsg) {
		if (!isForUs(rawMsg, MaxCulBinding.MAXCUL_ADDRESS)) {
			return null;
		}
		
		MaxCulMsgType msgType = getMsgType(rawMsg);
		switch (msgType) {
		case ACK: return new AckMsg(rawMsg);
		case ADD_LINK_PARTNER: return new AddLinkPartnerMsg(rawMsg);
		case CONFIG_TEMPERATURES: return new ConfigTemperaturesMsg(rawMsg);
		case CONFIG_VALVE: return null;
		case CONFIG_WEEK_PROFILE: return null;
		case PAIR_PING: return new PairPingMsg(rawMsg);
		case PAIR_PONG: return new PairPongMsg(rawMsg);
		case PUSH_BUTTON_STATE: return new PushButtonMsg(rawMsg);
		case REMOVE_GROUP_ID: return null;
		case REMOVE_LINK_PARTNER: return null;
		case RESET: return new ResetMsg(rawMsg);
		case SET_COMFORT_TEMPERATURE: return null;
		case SET_DISPLAY_ACTUAL_TEMP: return null;
		case SET_ECO_TEMPERATURE: return null;
		case SET_GROUP_ID: return new SetGroupIdMsg(rawMsg);
		case SET_TEMPERATURE: return new SetTemperatureMsg(rawMsg);
		case SHUTTER_CONTACT_STATE: return new ShutterContactMsg(rawMsg);
		case THERMOSTAT_STATE: return new ThermostatStateMsg(rawMsg);
		case TIME_INFO: return new TimeInfoMsg(rawMsg);
		case UNKNOWN: return null;
		case WAKEUP: return new WakeupMsg(rawMsg);
		case WALL_THERMOSTAT_CONTROL: return new WallThermostatControlMsg(rawMsg);
		case WALL_THERMOSTAT_STATE: return new WallThermostatStateMsg(rawMsg);
		default:
			logger.error("unhandled msg type: " + msgType);
			return null;
		}
	}
