...
public class AddLinkPartnerMsg extends BaseMsg {
	...
	public AddLinkPartnerMsg(String rawMsg) {
		super(rawMsg);
		this.partnerAddr = "";
		partnerAddr += String.format("%02x", super.payload[0]).toUpperCase();
		partnerAddr += String.format("%02x", super.payload[1]).toUpperCase();
		partnerAddr += String.format("%02x", super.payload[2]).toUpperCase();
		devType = MaxCulDevice.getDeviceTypeFromInt(super.payload[3]);
	}
